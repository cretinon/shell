...
**E3. `_remove_french` — strip French accents** — new String Management helper (mirrors `_upper`/`_lower`: pipe-compatible via `${*:-$(</dev/stdin)}`, `local LC_ALL=C`, telemetry-free) that replaces every French accented letter with its ASCII base letter: `à â ä é è ê ë î ï ô ö ù û ü ÿ ç` and uppercase `À Â Ä É È Ê Ë Î Ï Ô Ö Ù Û Ü Ÿ Ç`. Implemented with bash parameter expansion (`${__input//à/a}` ...) — a `sed y/` transliteration was considered and rejected because under `LC_ALL=C` GNU sed treats UTF-8 as raw bytes and fails with "strings for `y' command are different lengths". Ligatures (`œ`, `æ`) are intentionally left unchanged (they are not accents).
> **STATUS: DONE** — added in `lib_shell.sh` String Management section (after `_lower`); `AGENTS.md` architecture bullet and `functions.md` updated; BATS cases added (all accents, pipe, empty input, no-op on plain text, chained with `_upper`).

**E4. GIT section in `lib_shell.sh` — reusable git query helpers** — new `### GIT ###` section (previously an empty banner) with four instrumented helpers that wrap the git commands previously inlined in `mcp`'s `__mcp_commit_push_note` / `__mcp_commit_message`: `_git_upstream` (echoes the upstream branch ref of the current branch, e.g. `origin/main`), `_git_commits_ahead` (echoes the count of local commits ahead of `@{u}`), `_git_staged_shortstat` (echoes the staged-diff shortstat, e.g. `2 files changed, 5 insertions(+), 1 deletion(-)`), `_git_staged_stat` (echoes the full staged-diff stat block). Each validates `$1` (`_error "DIR EMPTY"` → ret `10`), returns `1` with an `_error` on git failure (no upstream / not a git repository), and echoes the result on success (ret `0`).
> **STATUS: DONE** — added in `lib_shell.sh` GIT section (between STRING MANAGEMENT and URL); `AGENTS.md` architecture bullet and `functions.md` GIT section updated; 16 BATS cases cover nominal + edge branches (upstream present/absent, ahead 0/N, staged/empty shortstat+stat, non-git dir, empty arg). `mcp` `__mcp_commit_push_note` / `__mcp_commit_message` refactored to delegate to these helpers.

**E5. GIT section — work-tree / status / raw-diff helpers** — three more instrumented helpers in the `### GIT ###` section of `lib_shell.sh`, completing the query set used by `mcp_server.sh`: `_git_is_work_tree` (predicate, ret `0` when the dir is inside a git work tree, `1` otherwise), `_git_porcelain_status` (echoes `git status --porcelain`, empty when clean), `_git_diff` (echoes the raw git diff; `$2` optional: `HEAD`, `--cached`, or plain `git diff`). Same conventions as E4: `_error "DIR EMPTY"` → ret `10`, `_error` + ret `1` on git failure.
> **STATUS: DONE** — added in `lib_shell.sh` GIT section (after `_git_staged_stat`); `AGENTS.md` architecture bullet and `functions.md` GIT section updated; 13 BATS cases cover nominal + edge branches (work tree present/absent, porcelain clean/dirty, diff HEAD/--cached/plain, no changes, non-git dir, empty arg). `mcp_server.sh` `_mcp_tool_commit_all` / `_mcp_prompt_commit_message` / `_mcp_tool_commit_message` refactored to delegate to these helpers.

**E6. GIT section — stage/commit helpers + `_installed "git"` hardening** — two more instrumented helpers in the `### GIT ###` section of `lib_shell.sh`: `_git_add` (stages all changes, `git add -A`) and `_git_commit` (commits staged changes with a message, `git commit -m`). **All 9 GIT helpers now start with `_installed "git"` → `_error "GIT: not found"` → ret `10`**, so they fail loudly instead of shelling out to a missing binary.
> **STATUS: DONE** — added `_git_add`/`_git_commit` after `_git_diff`; `_installed "git"` check added to all 9 helpers; `AGENTS.md` architecture bullet and `functions.md` GIT section updated (all 9 entries document the not-installed case); BATS cases cover stage/commit nominal + edge branches and the not-installed branch of all 9 helpers. `mcp_server.sh` `_mcp_tool_commit_all` no longer calls `git` directly (stage + commit delegate to the helpers) — the `mcp` project's production code has zero direct `git` calls.

**E7. `_bats` — optional regex filter to run a subset of the tests** — the `-b`/`--bats` CLI action and the `_bats` function accept an optional single filter regex that is forwarded to `bats --filter`, so the whole `tests.bats` suite no longer has to run on every iteration when only one test (or a few) is being worked on. Selection: one test via a unique substring or an anchored `^exact name$`, several tests via an alternation such as `'name1|name2'`. `_process_opts` forwards the trailing positional argument to `_bats "$@"`; more than one argument is rejected (`too many arguments...`, ret `10`); `-b` alone and `_kcov`'s internal full-suite run keep their behavior.
> **STATUS: DONE** — working tree (commit hash to be added on commit); `AGENTS.md` architecture bullet, `functions.md` `_bats` entry updated; 3 BATS cases added (filter forwarded, no `--filter` without argument, more-than-one-argument rejected).

**E8. `_curl` intercepts 5 more curl exit codes** — the `case $__return in` error handler in `_curl` only mapped codes `3` (URL malformed), `6` (DNS) and `35` (SSL); every other curl failure fell into the generic `Something went wrong in _curl` branch. Five more exit codes are now intercepted with specific `_error` messages while still returning the original curl code: `5` (`Proxy error for _curl` — could not resolve proxy), `7` (`Connection error for _curl` — failed to connect to host), `23` (`Write error for _curl`), `26` (`Read error for _curl`), `47` (`Too many redirects for _curl`).
> **STATUS: DONE** — working tree (commit hash to be added on commit); `functions.md` `_curl` entry Returns updated; 5 BATS cases added (code 5/7/23/26/47 → status equals the code and the specific message is echoed).

**E9. `_curl` intercepts HTTP error status codes (400/401/403/404/500/502/503/504)** — `_curl` previously intercepted only a single HTTP status (`504 Gateway Time-out`) and relied on a body-text `grep "Unauthorized"` to report `TOKEN invalid`. The 504 `if` is now a `case` on the `--write-out`-appended status code (guarded by curl exit `0`), and every error status fails loudly with its standard reason phrase: `400 Bad Request`, `401 Unauthorized`, `403 Forbidden`, `404 Not Found`, `500 Internal Server Error`, `502 Bad Gateway`, `503 Service Unavailable`, `504 Gateway Time-out` — each `_error` + ret `1`. The body-grep `TOKEN invalid` branch was **removed** (status-only detection): a real invalid token now surfaces as `401 Unauthorized`, and an HTTP `200` body containing the literal text `Unauthorized` is treated as a success. Downstream callers (`lib_storm.sh`, `mcp_server.sh`) branch only on `_curl`'s non-zero return, so they are unaffected. The curl exit-code branches (`3`/`5`/`6`/`7`/`23`/`26`/`35`/`47`) are unchanged.
> **STATUS: DONE** — working tree (commit hash to be added on commit); `functions.md` `_curl` entry Description/Returns updated; BATS: the old `response contains Unauthorized` test replaced by `response is HTTP 401`, plus 6 new status tests (400/403/404/500/502/503) mirroring the 504 one.
