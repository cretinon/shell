...
_kcov () {
    ...
    if ! $DRY_RUN ; then
        kcov --exclude-path="$MY_GIT_DIR/shell/.git/,$MY_GIT_DIR/shell/README.md,/usr/,$MY_GIT_DIR/shell/.codecov.yml" --include-path="$MY_GIT_DIR/shell" "$__tmp" "$MY_GIT_DIR/shell/my_warp.sh" --lib "$LIB" -b
